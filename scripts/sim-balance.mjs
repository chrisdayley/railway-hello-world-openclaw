...
const list=raw.filter(p=>p && (p.purchasePrice!==undefined || p.price!==undefined || p.purchase!==undefined));
const N=10000;
function avg(a){return a.reduce((x,y)=>x+y,0)/Math.max(1,a.length)}
console.log('=== Economy Monte Carlo ===');
for(const p of list){const min=p.baseRevenueMin??p.revenueMin??p.min??p.revenue?.[0]??0,max=p.baseRevenueMax??p.revenueMax??p.max??p.revenue?.[1]??0,cost=p.operatingCost??p.cost??0,price=p.purchasePrice??p.price??p.purchase??0;if(!max)continue;let vals=[];for(let i=0;i<N;i++)vals.push(min+Math.random()*(max-min)-cost);let daily=avg(vals),payback=daily>0?price/daily:Infinity;console.log(`${p.name||p.id}: avg ${daily.toFixed(1)}g/day, payback ${payback.toFixed(1)} days`)}
console.log('\n=== Loot rarity sample ===');
