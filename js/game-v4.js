(() => {
  ...
  function attack() {
    ...
    const d=aimDirection(112),range=combo===3?92:76;let dmg=combatStats().power*(combo===3?1.55:1)*(counterBuff>0?1.65+skillBonuses().counterPower:1)*(S.settings.assistMode?1.25:1);counterBuff=0;
  ...
  function castSpell(){
    ...
    const d=aimDirection(360);projectiles.push({kind:'player',x:S.x+d.x*24,y:S.y+d.y*16,vx:d.x*360,vy:d.y*360,r:9,life:1.7,damage:combatStats().spellDamage,chain:hasSkill('chain_light')});
  }
  function dodge(){
    if(gameplayBlocked()||dodgeCd>0||S.stam<24*(1-skillBonuses().dodgeCostReduction))return;wakeAudio();dismissTutorial();const threatened=enemies.some(e=>e.state==='windup'&&distance(e,S)<e.r+110);const d=facing();S.stam-=24*(1-skillBonuses().dodgeCostReduction);dodgeCd=.82;invuln=.5;moveActor(S,d.x*78,d.y*78,13);if(threatened&&hasSkill('counter_light')){counterBuff=4;toast('Counterlight ready');}haptic(12);tone(260,.1,'triangle',.02,130);
  ...
  function solidRects(){const z=zone();if(INT&&z.type==='room'&&z.theme!=='dungeon')return INT.solids(roomOptions());return INTERIOR_LAYOUTS[S.zone]?.solids||EX?.roomSolids[S.zone]||(/^(home|business)@/.test(S.zone)?EX.roomSolids.bellkeeper:[]);}
  ...
  function companionActive(){return !!S.storyChoice&&(['greenwake','moonfall'].includes(S.zone)||S.story?.flags.miraRoads&&!!atlasArea());}
  ...
  if(['localhost','127.0.0.1'].includes(location.hostname)) window.__EVERLIGHT_DEBUG__ = {
    navigation:{
      ...
      get:()=>({state:structuredClone(S),enemies:structuredClone(enemies),projectiles:structuredClone(projectiles),loot:structuredClone(loot),target:navigationTarget(),camera:{...camera},paused,sprinting,doorCooldown}),
