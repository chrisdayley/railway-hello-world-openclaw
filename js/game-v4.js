(() => {
  ...
  const BUILD = '29';
  const HEARTH = window.EVERLIGHT_HEARTH;
  const STORY = window.EVERLIGHT_STORY;
  const EX = window.EVERLIGHT_EXPLORATION;
  const ATLAS = window.EVERLIGHT_ATLAS || null;
  const ECON = window.EVERLIGHT_ECONOMY;
  const PROGRESSION = window.EVERLIGHT_PROGRESSION;
  const INT = window.EVERLIGHT_INTERIORS;
  ...
  const el = Object.fromEntries([
    ...
    'fullscreenBtn','pauseBtn','runBtn','mapBtn','minimapCanvas','minimapLabel','destinationHint','dialogue','speakerPortrait','speakerName','dialogueText','dialogueChoices',
  ...
  const ITEM_TEMPLATES = {
    ...
    echo_buckler: { id: 'echo_buckler', name: 'Echo Buckler', type: 'Shield', rarity: 'Legendary', armor: 7, maxHp: 18, value: 720, description: 'A guardian’s shield, earned by recovering a lost keeper’s oath.' },
    aether_lens: { id: 'aether_lens', name: 'Aether Lens', type: 'Quest', rarity: 'Rare', value: 0, description: 'Story relic—not equipment. The first piece of a machine that can cross the upper sky.' }
  };
  ...
  Object.assign(ITEM_TEMPLATES,{
    traveler_helm:{id:'traveler_helm',name:'Traveler’s Hood',type:'Helm',rarity:'Common',armor:1,maxHp:4,value:35,description:'Stitched wool and a discreet leather lining.'},
    trail_helm:{id:'trail_helm',name:'Trailwatch Helm',type:'Helm',rarity:'Uncommon',armor:3,maxHp:6,value:95,description:'A scout’s light, open-faced helm.'},
    rune_helm:{id:'rune_helm',name:'Runekeeper’s Circlet',type:'Helm',rarity:'Rare',armor:3,spell:.08,maxHp:10,value:240,description:'Weathered runes focus the bearer’s aether.'},
    warden_helm:{id:'warden_helm',name:'Warden’s Visor',type:'Helm',rarity:'Epic',armor:6,maxHp:18,value:480,description:'A trophy from a guardian that never yielded.'},
    crown_of_echoes:{id:'crown_of_echoes',name:'Crown of Echoes',type:'Helm',rarity:'Legendary',armor:7,spell:.14,maxHp:22,value:980,description:'Won from the most dangerous keepers of the old roads.'},
    road_belt:{id:'road_belt',name:'Roadworn Belt',type:'Belt',rarity:'Common',armor:1,maxHp:4,value:28,description:'Reliable leather, many repairs, room for another journey.'},
    scout_belt:{id:'scout_belt',name:'Scout’s Sash',type:'Belt',rarity:'Uncommon',armor:2,maxHp:6,crit:.01,value:80,description:'A balanced kit for quick footwork.'},
    duelist_belt:{id:'duelist_belt',name:'Duelist’s Baldric',type:'Belt',rarity:'Rare',armor:3,maxHp:10,crit:.025,value:220,description:'Keeps your blade exactly where it belongs.'},
    starwoven_belt:{id:'starwoven_belt',name:'Starwoven Girdle',type:'Belt',rarity:'Legendary',armor:5,maxHp:20,crit:.04,value:950,description:'Starlight threaded through an ancient champion’s sash.'},
    oak_shield:{id:'oak_shield',name:'Ironbound Oak Shield',type:'Shield',rarity:'Uncommon',armor:3,maxHp:6,value:105,description:'An honest shield reinforced with iron bands.'},
    sentinel_shield:{id:'sentinel_shield',name:'Sentinel’s Shield',type:'Shield',rarity:'Rare',armor:5,maxHp:12,value:280,description:'Carried by the watch on the longest nights.'},
    sunward_shield:{id:'sunward_shield',name:'Sunward Aegis',type:'Shield',rarity:'Epic',armor:7,maxHp:16,value:520,description:'A boss-forged shield that carries the morning’s warmth.'}
  });
  const SKILLS = [
  ...
  const SHOP_STOCK = {
    smithy: ['briar_edge','leather_coat','moonfall_saber','oak_shield','traveler_helm','road_belt'],
    apothecary: ['tonic','tonic','aether_draught'],
    archive: ['lantern_charm','aether_draught'],
    road: ['tonic','aether_draught','wayfarer_bow','leather_coat','trail_helm','scout_belt']
  };
  ...
  function zoneById(id) {
    if(HEARTH.rooms[id])return HEARTH.rooms[id];
  ...
  const defaultSave = () => ({
    schema: 9, build: BUILD, style: null, zone: 'northford', x: 965, y: 625,
    ...
    equipment: { Weapon: 'starter-weapon', Armor: null, Shield:null, Helm:null, Belt:null, Charm1:null, Charm2:null }, materials: { moonleaf: 0, briarFiber: 0, wardenAlloy: 0 },
    ...
    story: {version:1,step:0,kills:0,flags:{},choices:{},side:{},tracked:'main'},
  ...
  function migrateSave(raw) {
    ...
    const merged = { ...base, ...raw, schema: 9, build: BUILD };
    ...
    // Preserve earned items; the old buckler becomes a shield, never disappears.
    const oldBuckler=merged.inventory.find(i=>i.id==='echo_buckler'&&i.type==='Charm');
    if(oldBuckler){oldBuckler.type='Shield';if([raw.equipment?.Charm,raw.equipment?.Charm1,raw.equipment?.Charm2].includes(oldBuckler.uid)&&!merged.equipment.Shield)merged.equipment.Shield=oldBuckler.uid;}
    window.EVERLIGHT_EQUIPMENT.migrate(merged);
    ...
    STORY.init(merged);HEARTH.init(merged);
    window.EVERLIGHT_SKILLS.migrate(merged);
    return merged;
  }
  function loadSave() {
    try {
  ...
  function enterZone(id, spawn = null, announceZone = true) {
    ...
    ensureStoryEncounter();spawnHearthEncounter();
    updateCamera(0,true);
  ...
  function makeEnemy(kind, x, y, index = 0) {
    const stats = {
      ...
      warden: { hp: 1380, r: 42, speed: 118, damage: 39, xp: 328, telegraph: .8, recover: .72 }
    }[kind];
  ...
  function spawnZoneEnemies() {
    ...
    if(['cistern','sluice','vault'].includes(S.zone)){
      ...
      if(S.zone==='vault'&&!S.opened.includes('drowned-keeper'))enemies.push(configureBoss(makeEnemy('warden',560,350,90)));
      return;
    }
    if (S.zone === 'greenwake') {
    ...
    if (S.zone === 'moonfall') {
      ...
      if (S.mainStep >= 6 && !S.opened.includes('hollow-warden')) enemies.push(configureBoss(makeEnemy('warden', 1570, 560, 9)));
    }
    ...
    if (area) {
      ...
      if (area.kind!=='settlement' && area.hasElite && !S.opened.includes(`${area.id}:elite`)) {
        const elite = makeEnemy('warden', 900, 480, 99);
        enemies.push(configureBoss(elite));
      }
    }
  }
  function configureBoss(e,rematch=false){const p=PROGRESSION.profile(S.zone,atlasArea(),rematch);Object.assign(e,{hp:p.hp,maxHp:p.hp,speed:p.speed,damage:p.damage,xp:p.xp,recommendedLevel:p.level,bossName:p.name,resistance:p.armor,rematch,cycle:0,engaged:false});return e;}
  ...
  function hasSkill(id) { return window.EVERLIGHT_SKILLS.has(S,id); }
  function skillBonuses(){return window.EVERLIGHT_SKILLS.bonuses(S);}
  ...
  function gainXp(amount) {
    ...
    S.xp += amount;
    while (S.level<100 && S.xp >= S.level * 80) {
      S.xp -= S.level * 80; S.level++; S.skillPoints++; S.maxHp += 8; S.hp = combatStats().maxHp; S.maxMana += 4; S.mana = combatStats().maxMana; S.stam=combatStats().maxStamina;
      toast(`Level ${S.level} · Skill point earned`); sfx('success');
    }
    if(S.level>before){const old=S.pendingLevelUp;S.pendingLevelUp={from:old?.from||before,to:S.level,points:(old?.points||0)+S.level-before};save();}
    if(S.level>=100)S.xp=0;
  }
  function presentLevelUp(){if(!S.pendingLevelUp||paused||!running)return;const p=S.pendingLevelUp;paused=true;el.gameScreen.inert=true;input={x:0,y:0};keys.clear();$('levelUpNumber').textContent=p.to;$('levelUpGains').textContent=`+${p.points} skill point${p.points===1?'':'s'} · +${p.points*8} health · +${p.points*4} aether · Fully restored`;$('levelUpPanel').classList.toggle('level-up--still',!!S.settings.reducedMotion);show($('levelUpPanel'));$('levelUpContinue').focus();tone(523,.22,'sine',.04,784);announce(`Level ${p.to}! ${p.points} skill points earned.`);}
  function dismissLevelUp(skills=false){show($('levelUpPanel'),false);delete S.pendingLevelUp;el.gameScreen.inert=false;paused=false;last=performance.now();save();if(skills)openJournal('skills');}
  ...
  function playerDamage(amount, sourceX, sourceY) {
    if (invuln > 0 || gameplayBlocked()) return;
    const reduced = Math.max(1, Math.round(amount * (1-combatStats().damageReduction) * (S.settings.assistMode ? .6 : 1)));
    S.hp = Math.max(0, S.hp - reduced); invuln = 1.05; hurtFlash = .24; shake = 8; sfx('hurt');
  ...
  function gameplayBlocked(){return paused||[el.dialogue,el.journal,el.chapterComplete,el.interactionPanel,$('levelUpPanel')].some(n=>n&&!n.classList.contains('is-hidden'));}
  function defeatPlayer() {
    paused = true; subtitle('The Way pulls you back from the dark…', 2.2); screenFlash = .65;
    setTimeout(() => { S.hp=combatStats().maxHp; S.mana=combatStats().maxMana; S.stam=combatStats().maxStamina; S.gold=Math.max(0,S.gold-12); enterZone('northford', ZONES.northford.spawn); paused=false; toast('Returned to Northford · −12 gold'); save(); }, 1500);
  }
  function hitEnemy(e, damage, dx, dy, magic = false, canKill = true) {
    if (e.dead) return; damage*=1-(e.resistance||0);if(!canKill)damage=Math.min(damage,Math.max(0,e.hp-1));e.hp -= damage; e.flash = .13;e.engaged=true;moveActor(e,dx*(e.kind==='warden'?1:8),dy*(e.kind==='warden'?1:8),e.r*.65);
  ...
  function dropLoot(e) {
    const boss = e.kind === 'warden'; const storyBoss = boss && S.zone==='moonfall'&&!e.rematch&&!S.opened.includes('hollow-warden'); const coins = boss ? 20 : e.kind === 'construct' ? 7 : 5;
    ...
    if (storyBoss) { makeDrop('item',e.x,e.y,{ itemId:'warden_plate',rarity:'Epic' }); makeDrop('item',e.x,e.y,{ itemId:'aether_lens',rarity:'Rare' }); }
    else if (boss) { const reward=PROGRESSION.treasure({source:'boss',danger:atlasArea()?.danger||3});makeDrop('item',e.x,e.y,{itemId:reward.itemId,rarity:reward.tier}); }
    else if (Math.random() < .2 + rareBonus) {const reward=PROGRESSION.treasure({source:'enemy',danger:atlasArea()?.danger||1});makeDrop('item',e.x,e.y,{itemId:reward.itemId,rarity:reward.tier});}
  }
  function killEnemy(e) {
    ...
    if (e.kind==='warden' && S.zone==='moonfall'&&!e.rematch) { if(!S.opened.includes('hollow-warden'))S.opened.push('hollow-warden'); S.mainStep=Math.max(S.mainStep,7); sfx('success'); subtitle('The Warden falls. Claim the Aether Lens from its hoard.',3); }
    if(e.rematch){S.opened.push('warden-rematch-v27');save();subtitle('The old guardian acknowledges your mastery.',3);}
    ...
    if(e.memoryBoss){S.hearth.stage=4;hearthHazards=[];projectiles=projectiles.filter(p=>p.kind!=='enemy');save();subtitle('The silence breaks. Someone behind it is saying Mira’s name.',5);}
    STORY.kill(S,e,storyHooks());
    const area=atlasArea();
    if(area){
  ...
  function attack() {
    if(gameplayBlocked()||attackCd>0)return;wakeAudio();dismissTutorial();combo=comboClock>0?combo%3+1:1;comboClock=.55;attackCd=combo===3?.42:.27;invuln=Math.max(invuln,.08);sfx('sword');
    if(equipped('Weapon')?.id==='wayfarer_bow'){const d=aimDirection(460);attackCd=.48;projectiles.push({kind:'player',arrow:true,x:S.x+d.x*24,y:S.y+d.y*16,vx:d.x*480,vy:d.y*480,r:5,life:1.25,damage:combatStats().power*(resonanceReady>0?1.35:1)});resonanceReady=0;return;}
    const d=aimDirection(112),range=combo===3?92:76;let dmg=combatStats().power*(combo===3?1.55:1)*(counterBuff>0?1.65+skillBonuses().counterPower:1)*(S.settings.assistMode?1.25:1)*(resonanceReady>0?1.35:1);counterBuff=0;resonanceReady=0;
  ...
  function castSpell(){
    if(gameplayBlocked()||spellCd>0)return;wakeAudio();dismissTutorial();const cost=16;if(S.mana<cost){toast('Not enough aether');return;}S.mana-=cost;spellCd=1.05;sfx('spell');
    const d=aimDirection(360);projectiles.push({kind:'player',x:S.x+d.x*24,y:S.y+d.y*16,vx:d.x*360,vy:d.y*360,r:9,life:1.7,damage:combatStats().spellDamage*(resonanceReady>0?1.35:1),chain:hasSkill('chain_light')});resonanceReady=0;
  }
  function dodge(){
    if(gameplayBlocked()||dodgeCd>0||S.stam<24*(1-skillBonuses().dodgeCostReduction))return;wakeAudio();dismissTutorial();const threatened=enemies.some(e=>e.state==='windup'&&distance(e,S)<e.r+110);const d=facing();resonate(threatened||hearthHazards.some(p=>!p.hit&&distance(S,p)<p.r+20));S.stam-=24*(1-skillBonuses().dodgeCostReduction);dodgeCd=.82;invuln=.5;moveActor(S,d.x*78,d.y*78,13);if(threatened&&hasSkill('counter_light')){counterBuff=4;toast('Counterlight ready');}haptic(12);tone(260,.1,'triangle',.02,130);
  ...
  function roomOptions(){const z=zone();return {zoneId:S.zone,name:z.name,building:z.building,businessType:z.estateId?S.properties[z.estateId]?.businessType:null,region:ATLAS.get(z.parent||z.estateParent)};}
  function solidRects(){if(HEARTH.inside(S))return HEARTH.solids(S);const z=zone();if(INT&&z.type==='room'&&z.theme!=='dungeon')return INT.solids(roomOptions());return INTERIOR_LAYOUTS[S.zone]?.solids||EX?.roomSolids[S.zone]||(/^(home|business)@/.test(S.zone)?EX.roomSolids.bellkeeper:[]);}
  ...
  function updatePlayer(dt){
    ...
    const pace=sprinting?1.7:1;moveActor(S,ix*150*combatStats().speed*pace*dt,iy*150*combatStats().speed*pace*dt,13);S.stam=clamp(S.stam+(sprinting&&danger?-18:28*(1+skillBonuses().staminaRegen))*dt,0,combatStats().maxStamina);S.mana=Math.min(combatStats().maxMana,S.mana+4.5*(1+skillBonuses().manaRegen)*dt);
  ...
  function startEnemyWindup(e, dx, dy, d) {
    ...
    if(e.kind==='warden'){const plan=PROGRESSION.attackPlan(e,d);e.phase=plan.phase;e.attackKind=plan.kind;e.telegraphDuration=plan.windup;e.stateTimer=plan.windup;e.recoverDuration=plan.recovery;e.nextCooldown=plan.cooldown;e.attackDuration=plan.attackTime;e.cycle=(e.cycle||0)+1;e.engaged=true;}else e.attackKind=e.kind==='wisp'?'bolt':'melee';
  }
  function resolveEnemyAttack(e) {
    e.state='attack';e.stateTimer=e.attackDuration||(e.attackKind==='melee'?.2:.28);e.didHit=false;shake=Math.max(shake,e.kind==='warden'?6:2);
    ...
    if(e.attackKind==='volley'){const base=Math.atan2(e.attackDir.y,e.attackDir.x),count=e.phase===2?5:3;for(let i=0;i<count;i++){const a=base+(i-(count-1)/2)*.18;projectiles.push({kind:'enemy',x:e.x+Math.cos(a)*e.r,y:e.y+Math.sin(a)*e.r,vx:Math.cos(a)*310,vy:Math.sin(a)*310,r:9,life:3.2,damage:e.damage*.85});}}
    if(e.attackKind==='radial')for(let i=0;i<12;i++){const a=i*Math.PI/6;projectiles.push({kind:'enemy',x:e.x,y:e.y,vx:Math.cos(a)*225,vy:Math.sin(a)*225,r:8,life:2.8,damage:e.damage*.75});}
  }
  function updateEnemies(dt){
    for(const e of enemies){if(e.dead)continue;e.flash=Math.max(0,e.flash-dt);e.attackCd=Math.max(0,e.attackCd-dt);const dx=S.x-e.x,dy=S.y-e.y,d=Math.hypot(dx,dy)||1,nx=dx/d,ny=dy/d;if(zoneGrace>0){e.state='idle';continue;}
      ...
      if(e.state==='attack'){
        ...
        if(['melee','slam','charge'].includes(e.attackKind)){const pace=e.attackKind==='charge'?4.6:e.attackKind==='slam'?1.8:3.2;moveActor(e,e.attackDir.x*e.speed*pace*dt,e.attackDir.y*e.speed*pace*dt,e.r*.7);}
        if(!e.didHit&&['melee','slam','charge'].includes(e.attackKind)&&distance(e,S)<e.r+(e.attackKind==='slam'?72:28)){playerDamage(e.damage,e.x,e.y);e.didHit=true;}
        if(e.stateTimer<=0){e.state='recover';e.stateTimer=e.recoverDuration;e.attackCd=e.kind==='warden'?e.nextCooldown:random(.7,1.2);}continue;
      }
      if(e.state==='recover'){e.stateTimer-=dt;if(e.stateTimer<=0)e.state='chase';continue;}
      const reach=e.kind==='warden'?780:e.kind==='wisp'?150:e.r+38;if(d<reach&&e.attackCd<=0){startEnemyWindup(e,dx,dy,d);continue;}
      if(d<(e.kind==='warden'?1000:440)||e.engaged){moveActor(e,nx*e.speed*(e.kind==='warden'&&e.phase===2?1.2:1)*dt,ny*e.speed*(e.kind==='warden'&&e.phase===2?1.2:1)*dt,e.r*.65);e.state='chase';}else e.state='idle';
      if(e.kind==='warden')e.phase=e.hp<e.maxHp*.45?2:1;
    }
  }
  function companionActive(){if(S.hearth?.stage===5&&S.hearth.choice==='road'&&(HEARTH.inside(S)||['wild','ruins','atlas'].includes(zone().type)))return true;if(HEARTH.inside(S))return S.zone==='memory_heart'&&S.hearth?.stage<5;return !!S.storyChoice&&(['greenwake','moonfall'].includes(S.zone)||S.story?.flags.miraRoads&&!!atlasArea());}
  ...
  function updateLoot(dt){
    for(const d of loot){if(d.collected)continue;d.age+=dt;if(d.age<.55){d.x+=d.vx*dt;d.y+=d.vy*dt;d.vy+=180*dt;}else{d.vx*=.85;d.vy=0;}if(d.age>.45&&distance(d,S)<42*(1+(hasSkill('treasure_sense')?.4:0)+skillBonuses().lootRadius))collectDrop(d);}
  ...
  function objectsForZone(){
    const z=zone();
    if(HEARTH.inside(S))return HEARTH.objects(S);
    ...
    out.push(...STORY.objects(S),...HEARTH.objects(S));
    if(S.zone===ATLAS.firstArea)out.push({id:'returnMoonfall',kind:'portal',name:'Moonfall Ruins',x:70,y:550,label:'Return',target:'moonfall',spawn:{x:1740,y:620}});
  ...
  function baseObjectsForZone(){
    ...
    if(S.zone==='moonfall')return [
      ...
      ...(S.opened.includes('hollow-warden')&&!S.opened.includes('warden-rematch-v27')?[{id:'warden-memory',kind:'trial',name:'Warden’s Remembrance · Level 6',x:1570,y:560,label:'Challenge'}]:[]),
    ...
    if (area) {
      ...
      for (let i=0;i<area.secretCount;i++) {
        ...
        const sealed=i===0&&area.kind==='dungeon'&&area.hasElite;
        out.push({id:`${area.id}:secret:${i}`,kind:'chest',secret:true,sealed,name:sealed?'Keeper’s Sealed Hoard':'Travel Cache',x:210+(seed%1380),y:270+((seed>>>9)%570),label:'Open'});
      }
      if(area.kind==='dungeon'&&area.hasElite)out.push(...[{x:300,y:840,suffix:'dawn'},{x:1490,y:280,suffix:'dusk'}].map(p=>({id:`${area.id}:seal:${p.suffix}`,kind:'seal',name:`${pretty(p.suffix)} Inscription`,x:p.x,y:p.y,label:S.opened.includes(`${area.id}:seal:${p.suffix}`)?'Read':'Attune'})));
  ...
  function interact(){
    ...
    if(o.kind==='hearth'){hearthInteract(o);return;}
    if(o.kind==='story'){const panel=STORY.panel(S,o.storyId);openInteraction(panel.title,panel.html);return;}
    ...
    if(o.kind==='seal'){if(!S.opened.includes(o.id)){S.opened.push(o.id);gainXp(18);save();}toast('A seal answers. Attune both inscriptions and defeat the keeper to open the hoard.');return;}
    if(o.kind==='trial'){if(enemies.some(e=>e.kind==='warden'&&!e.dead)){toast('The remembrance is already awake.');return;}openInteraction('Warden’s Remembrance',`<p>A one-time mastery challenge for completed Chapter I saves. Recommended level 6 with upgraded gear. Dodge the gold charge lane, sidestep red volleys, and strike during recovery.</p><p>Your chapter completion stays intact. Victory awards a boss hoard.</p><button data-boss-trial>Awaken the Warden</button>`);return;}
    ...
    if(o.kind==='rest'){S.hp=combatStats().maxHp;S.mana=combatStats().maxMana;S.stam=combatStats().maxStamina;const settled=advanceDay();toast(settled?'Rested · Business day settled':'Restored · Business days advance through adventuring, not repeated rest');save();return;}
  ...
  function talkMira(){
    if(S.mainStep===0)openDialogue([{speaker:'Mira',portrait:'M',text:'There you are. I found two cups on my table this morning. I live alone. Everyone says I always have. But someone has scratched a name under the second cup: Alden.'},{speaker:'Mira',portrait:'M',text:'The stone spoke that same name. Then the lanterns outside town went dark. Tell me what you heard. I need to know I am not inventing him.',choices:[{label:'“A voice asking for help.”',value:'mercy'},{label:'“A command: open the road.”',value:'power'}]}],choice=>{S.storyChoice=choice;S.mainStep=1;if(choice==='mercy')addItem('lantern_charm');else{S.maxMana+=15;S.mana=combatStats().maxMana;}save();closeDialogue();subtitle('Take the south gate. Find what is poisoning Greenwake.',3);});
  ...
  function openChest(o){
    ...
    if(o.sealed){const id=atlasArea()?.id;if(!S.opened.includes(`${id}:elite`)||!['dawn','dusk'].every(s=>S.opened.includes(`${id}:seal:${s}`))){toast('Sealed hoard · Find both inscriptions and defeat this dungeon’s keeper.');return;}}
    S.opened.push(o.id);
    ...
    const area=atlasArea(),reward=PROGRESSION.treasure({source:'chest',danger:area?.danger||1,sealed:!!o.sealed}),tier=reward.tier;
    const item=addItem(reward.itemId,tier);const gold=o.sealed?80+(area?.danger||1)*22:12+(area?.danger||1)*6;
  ...
  function priceFor(item){return Math.ceil(item.value*(S.faction==='gilded'?.9:1)*(S.story?.flags.roadDiscount?.95:1)*ECON.perk(S,item.type==='Consumable'?'apothecary':'smithy'));}
  function openInteraction(title,html){restoreFocus=document.activeElement;el.interactionTitle.textContent=title;el.interactionBody.innerHTML=html;el.interactionBody.scrollTop=0;paused=true;el.gameScreen.inert=true;show(el.interactionPanel);requestAnimationFrame(()=>el.closeInteraction.focus());}
  function closeInteraction(){show(el.interactionPanel,false);el.gameScreen.inert=false;paused=false;invuln=Math.max(invuln,1);zoneGrace=Math.max(zoneGrace,.75);input={x:0,y:0};keys.clear();last=performance.now();restoreFocus?.focus?.();restoreFocus=null;}
  let merchantContext=null,merchantView={mode:'buy',pendingUid:null,message:''};
  function renderMerchant(){if(!merchantContext)return;openInteraction(`${merchantContext.keeper} · Trade`,window.EVERLIGHT_MERCHANTS.render({state:S,stock:SHOP_STOCK[merchantContext.kind],templates:ITEM_TEMPLATES,priceFor,keeper:merchantContext.keeper,view:merchantView}));}
  function openShop(kind,keeper){merchantContext={kind,keeper};merchantView={mode:'buy',pendingUid:null,message:''};renderMerchant();}
  ...
  function advanceDay(){
    if((S.economyClock||0)<300)return false;
  ...
  function showChapterComplete(){STORY.init(S);S.story.tracked='main';save();el.chapterSummary.textContent='Chapter II is ready. Speak to Iona, the lantern courier beside the Moonfall Waystone. Your gold minimap marker leads to her, then follows every step of the investigation.';show(el.chapterComplete);el.gameScreen.inert=true;paused=true;el.keepExploringBtn.focus();}

  let hearthHazards=[],hearthStormClock=2.4,resonanceCd=0,resonanceReady=0;
  function openReturnRecap(){
    const [title,text]=objective();
    openInteraction(S.hearth.stage?'Your road is waiting':'A letter that should not exist',
      (S.hearth.stage?`<article class="hearth-card"><span class="choice-tag">Where you left off</span><h3>${title}</h3><p>${text}</p></article>`:'')+HEARTH.journal(S)+'<button data-hearth="keep">Continue where I was</button>');
  }
  function hearthHooks(){return {
    save,toast,voice:text=>subtitle(text,7),close:closeInteraction,panel:openInteraction,
    note:i=>tone([330,440,550][i],.38,'triangle',.05),success:()=>{sfx('success');screenFlash=.3;},
    reward:q=>storyHooks().reward(q),
    restore:()=>{S.hp=combatStats().maxHp;S.mana=combatStats().maxMana;S.stam=combatStats().maxStamina;save();},
    begin:()=>{if(!HEARTH.inside(S))S.hearth.returnTo={zone:S.zone,x:S.x,y:S.y};closeJournal();closeInteraction();S.activeMount=null;enterZone('memory_hall');save();},
    leave:()=>{const p=S.hearth.returnTo;closeInteraction();enterZone(p&&zoneById(p.zone)?p.zone:'northford',p&&zoneById(p.zone)?p:null);save();},
    travel:id=>{closeJournal();closeInteraction();enterZone(id);save();},
    anchor:()=>{const boss=enemies.find(e=>e.memoryBoss&&!e.dead);if(boss){boss.resistance=(3-S.hearth.anchors.length)*.18;boss.state='recover';boss.stateTimer=1.7;boss.attackCd=1.7;spawnBurst(S.x,S.y,'#ffcaa4',24);}sfx('success');}
  };}
  function hearthInteract(o){
    const direct=['leave','hall','workshop','heart','rain','bread','ember','west','east','north','stitch','rest'];
    if(direct.includes(o.hearth)){HEARTH.action(S,o.hearth,hearthHooks());updateHUD();return;}
    const p=HEARTH.panel(S,o.hearth);openInteraction(p.title,p.html);
  }
  function spawnHearthEncounter(){
    hearthHazards=[];hearthStormClock=2.4;
    if(S.zone!=='memory_heart'||S.hearth?.stage!==3)return;
    const level=S.hearth.level,e=configureBoss(makeEnemy('warden',600,470,290));
    Object.assign(e,{memoryBoss:true,bossName:'The Collector of Names',hp:600+level*100,maxHp:600+level*100,damage:14+level*2,speed:92+level*3,recommendedLevel:level,xp:100+level*12,resistance:(3-S.hearth.anchors.length)*.18});enemies.push(e);
  }
  function updateHearthCombat(dt){
    resonanceCd=Math.max(0,resonanceCd-dt);resonanceReady=Math.max(0,resonanceReady-dt);
    const boss=enemies.find(e=>e.memoryBoss&&!e.dead);
    if(!boss||S.zone!=='memory_heart')return;
    hearthStormClock-=dt;
    if(hearthStormClock<=0&&zoneGrace<=0){
      hearthStormClock=boss.hp<boss.maxHp*.5?2.7:3.8;
      hearthHazards.push({x:S.x,y:S.y,r:64,life:1.4,hit:false});
      if(boss.hp<boss.maxHp*.5)hearthHazards.push({x:clamp(S.x+lastFacing.x*125,150,1050),y:clamp(S.y+lastFacing.y*125,200,760),r:58,life:1.8,hit:false});
    }
    for(const p of hearthHazards){p.life-=dt;if(p.life<=.3&&!p.hit){p.hit=true;spawnBurst(p.x,p.y,'#eda6cb',16);if(distance(S,p)<p.r+10)playerDamage(boss.damage,p.x,p.y);}}
    hearthHazards=hearthHazards.filter(p=>p.life>0);
  }
  function resonate(threatened){
    if(!threatened||S.hearth?.stage!==5||resonanceCd>0)return;
    resonanceCd=4;resonanceReady=4;S.mana=Math.min(combatStats().maxMana,S.mana+8);
    if(S.hearth.choice==='home')S.hp=Math.min(combatStats().maxHp,S.hp+4);
    spawnBurst(S.x,S.y,'#ffe0a0',24);floatText(S.x,S.y-55,'RESONANCE · +8 AE','#ffe4ab');tone(660,.25,'triangle',.035,150);
  }
  function drawHearthHazards(){for(const p of hearthHazards){ctx.save();ctx.translate(p.x,p.y);ctx.fillStyle=p.hit?'#f2a8c888':'#ad62852c';ctx.strokeStyle=p.hit?'#fff2ca':'#ffa6d0';ctx.lineWidth=3;ctx.beginPath();ctx.arc(0,0,p.r,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.beginPath();ctx.arc(0,0,p.r*clamp(1-p.life/1.8,0,1),0,Math.PI*2);ctx.stroke();ctx.fillStyle='#ffe9d1';ctx.font='bold 12px system-ui';ctx.textAlign='center';ctx.fillText(p.hit?'BREAK':'MOVE',0,4);ctx.restore();}}
  function drawCollector(e){
    if(e.state==='windup')drawEnemyTelegraph(e);shadow(e.x,e.y+12,48);ctx.save();ctx.translate(e.x,e.y);
    ctx.fillStyle=e.flash>0?'#fff0d1':'#2c293f';ctx.strokeStyle='#cc9cb8';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(0,-100);ctx.lineTo(39,-65);ctx.lineTo(48,22);ctx.lineTo(12,4);ctx.lineTo(0,20);ctx.lineTo(-18,3);ctx.lineTo(-48,22);ctx.lineTo(-35,-65);ctx.closePath();ctx.fill();ctx.stroke();
    ctx.fillStyle='#ead5b3';ctx.fillRect(-17,-79,34,27);ctx.fillStyle='#392d46';ctx.fillRect(-12,-71,24,5);ctx.fillRect(-3,-61,6,7);
    for(let i=0;i<3;i++){ctx.fillStyle='#f4cca4';ctx.fillRect(-29+i*23,-34+(i%2)*8,13,19);}
    ctx.strokeStyle=e.resistance?'#d27aa9':'#77dccc';ctx.beginPath();ctx.ellipse(0,-28,60,72,0,0,Math.PI*2);ctx.stroke();ctx.restore();
  }
  function storyHooks(){return {item:id=>addItem(id),reward:q=>{S.gold+=q.gold||0;if(q.xp)gainXp(q.xp);if(q.item){const i=addItem(q.item);lootMessage(`${i.rarity} · ${i.name}`,i.rarity);}},changed:()=>{save();ensureStoryEncounter();updateHUD();toast(STORY.objective(S).join(' · '));}};}
  function ensureStoryEncounter(){const m=STORY.current(S);if(!m||m.kind!=='boss'||STORY.people[m.at].zone!==S.zone||enemies.some(e=>!e.dead&&e.storyBoss===S.story.step))return;enemies=enemies.filter(e=>e.kind!=='warden'||e.dead);const e=configureBoss(makeEnemy('warden',940,690,280));Object.assign(e,{storyBoss:S.story.step,bossName:m.boss,recommendedLevel:m.level,hp:m.hp,maxHp:m.hp,damage:25+m.level,speed:114+m.level});enemies.push(e);}
  function storyNavigation(objects){const t=STORY.target(S);if(!t)return null;const z=zone();if(t.zone===S.zone){if(t.kind==='kill')return enemies.find(e=>!e.dead)||objects.find(o=>o.storyId===t.id);if(t.kind==='boss')return enemies.find(e=>!e.dead&&e.storyBoss===S.story.step)||objects.find(o=>o.storyId===t.id);return objects.find(o=>o.storyId===t.id);}
    if(z.type==='interior'||z.type==='room')return objects.find(o=>o.id==='exit');
    const links=id=>id==='northford'?['greenwake']:id==='greenwake'?['northford','moonfall']:id==='moonfall'?['greenwake',ATLAS.firstArea]:id===ATLAS.firstArea?['moonfall',...Object.values(ATLAS.get(id).exits)]:Object.values(ATLAS.get(id)?.exits||{});
    const queue=[[S.zone,[]]],seen=new Set();while(queue.length){const [id,path]=queue.shift();if(seen.has(id))continue;seen.add(id);if(id===t.zone)return objects.find(o=>o.target===path[0]);for(const next of links(id))if(!seen.has(next))queue.push([next,[...path,next]]);}return null;
  }
  function pretty(text){return String(text).replace(/([A-Z])/g,' $1').replace(/_/g,' ').replace(/^./,c=>c.toUpperCase());}
  function objective(){if(HEARTH.inside(S))return HEARTH.objective(S);if(STORY.unlocked(S))return STORY.objective(S);const objectives=[['A Voice in the Vale','Find Mira near the lantern plaza'],['The South Road','Leave Northford through the south gate'],['Echoes in Greenwake',`Defeat corrupted echoes · ${Math.min(3,S.mainKills)}/3`],['A Mark in the Briars','Return to Mira in Northford'],['Choose an Ally','Visit the Guildhall and join a faction'],['Road to Moonfall','Take the east road through Greenwake'],['Keeper of the Seal','Defeat the Hollow Warden'],['The Fallen Warden','Collect the Aether Lens'],['The Road Remembers','Explore, grow stronger, and prepare for Chapter II']];return objectives[clamp(S.mainStep,0,objectives.length-1)];}
  function updateHUD(){const stats=combatStats(),hpMax=stats.maxHp;el.hpBar.style.width=`${clamp(S.hp/hpMax*100,0,100)}%`;el.manaBar.style.width=`${clamp(S.mana/stats.maxMana*100,0,100)}%`;el.stamBar.style.width=`${clamp(S.stam/stats.maxStamina*100,0,100)}%`;el.hpText.textContent=Math.ceil(S.hp);el.manaText.textContent=Math.ceil(S.mana);el.hpProgress.setAttribute('aria-valuemax',hpMax);el.hpProgress.setAttribute('aria-valuenow',Math.ceil(S.hp));el.manaProgress.setAttribute('aria-valuemax',stats.maxMana);el.manaProgress.setAttribute('aria-valuenow',Math.ceil(S.mana));el.stamProgress.setAttribute('aria-valuemax',stats.maxStamina);el.stamProgress.setAttribute('aria-valuenow',Math.ceil(S.stam));el.levelText.textContent=S.level;el.goldText.textContent=S.gold;const[t,x]=objective();el.objectiveTitle.textContent=t;el.objectiveText.textContent=x;const boss=enemies.find(e=>e.kind==='warden'&&!e.dead);show(el.bossHud,!!boss);if(boss){el.bossBar.style.width=`${Math.max(0,boss.hp/boss.maxHp*100)}%`;el.bossHud.querySelector('span').textContent=`${boss.bossName||'Waykeeper'} · Lv ${boss.recommendedLevel||6}${boss.phase===2?' · Unbound':''} · ${boss.state==='recover'?'COUNTER NOW':boss.state==='windup'?pretty(boss.attackKind):S.level<(boss.recommendedLevel||6)?'Train & upgrade':'Stay mobile'}`;}el.attackBtn.querySelector('small').textContent=counterBuff>0?'Counter':'Strike';el.runBtn.setAttribute('aria-pressed',String(runEnabled));el.runBtn.querySelector('small').textContent=sprinting?'Running':runEnabled?'Run on':'Run';if(trackedDoor&&S.zone===(trackedDoor.zone||'northford')){const dx=trackedDoor.x-S.x,dy=trackedDoor.y-S.y,d=Math.hypot(dx,dy);el.destinationHint.textContent=d<80?`${trackedDoor.name} · Walk onto the threshold`:`${Math.abs(dx)>Math.abs(dy)?dx>0?'→':'←':dy>0?'↓':'↑'} ${trackedDoor.name} · ${Math.round(d/10)} steps`;show(el.destinationHint,true);}else if(currentInteract?.door){el.destinationHint.textContent=`${currentInteract.name} · Walk in or tap ${currentInteract.label}`;show(el.destinationHint,true);}else if(HEARTH.inside(S)||STORY.unlocked(S)&&STORY.target(S)){const goal=navigationTarget();if(goal){const dx=goal.x-S.x,dy=goal.y-S.y;el.destinationHint.textContent=`${Math.abs(dx)>Math.abs(dy)?dx>0?'→':'←':dy>0?'↓':'↑'} ${goal.name||goal.bossName||'Quest target'} · ${Math.round(Math.hypot(dx,dy)/10)} steps`;show(el.destinationHint,true);}else show(el.destinationHint,false);}else show(el.destinationHint,false);void stats;}
  function navigationTarget(){
    const objects=objectsForZone(),z=zone();
    if(HEARTH.inside(S))return HEARTH.target(S,objects)||enemies.find(e=>e.memoryBoss&&!e.dead);
    if(trackedDoor&&S.zone===(trackedDoor.zone||'northford'))return trackedDoor;
    if(STORY.unlocked(S)){const target=storyNavigation(objects);if(target)return target;}
    if(z.type==='interior'||z.type==='room')return objects.find(o=>o.id==='exit');
    if(S.zone==='northford'){if(S.mainStep===0||S.mainStep===3)return objects.find(o=>o.id==='mira');if(S.mainStep===4)return objects.find(o=>o.id==='guildDoor');return objects.find(o=>o.id==='southGate');}
    if(S.zone==='greenwake'){if(S.mainStep===2)return enemies.find(e=>!e.dead)||objects.find(o=>o.id==='northGate');return objects.find(o=>o.id===(S.mainStep===3?'northGate':'moonfallRoad'));}
    if(S.zone==='moonfall'){if(S.mainStep===6)return enemies.find(e=>e.kind==='warden'&&!e.dead);if(S.mainStep===7)return loot.find(d=>d.itemId==='aether_lens');return objects.find(o=>o.id==='worldRoad')||objects.find(o=>o.kind==='waystone');}
    const area=atlasArea();if(area){
      const contract=Object.values(S.dynamicQuests).find(q=>q.status==='active'||q.status==='ready');
      const targetZone=trackedDoor?.zone|| (contract?(contract.status==='ready'?contract.areaId:contract.targetAreaId):null);
      if(targetZone===S.zone)return contract?.status==='ready'?objects.find(o=>o.kind==='notice'):enemies.find(e=>!e.dead);
      if(targetZone){const queue=[[area.id,[]]],seen=new Set();while(queue.length){const [id,route]=queue.shift();if(seen.has(id))continue;seen.add(id);if(id===targetZone)return objects.find(o=>o.target===route[0]);for(const next of Object.values(ATLAS.get(id)?.exits||{}))queue.push([next,[...route,next]]);}}
      return objects.find(o=>o.kind==='notice')||objects.find(o=>o.kind==='waystone')||objects.find(o=>o.kind==='portal'&&!S.atlasDiscovered.includes(o.target));
    }return null;
  }
  function updateMinimap(){if(!window.EVERLIGHT_MINIMAP)return;const z=zone(),target=navigationTarget();el.minimapLabel.textContent=target?.name||target?.bossName||(target?'Clear threats':z.name);el.mapBtn.setAttribute('aria-label',`${z.name} minimap. You are the cyan dot; gold diamond: ${el.minimapLabel.textContent}. Open map.`);window.EVERLIGHT_MINIMAP.draw(el.minimapCanvas,{zone:z,player:S,objects:objectsForZone(),enemies,solids:solidRects(),polygons:S.zone==='northford'?EX.townSolids:[],houses:EX.homes(atlasArea()),target,opened:S.opened,time:performance.now(),background:S.zone==='northford'&&imageReady.bg?bg:null});}
  ...
  function renderJournal(tab=currentTab){currentTab=tab;el.journalBody.classList.toggle('journal-body--gear',tab==='gear');el.journalBody.classList.toggle('journal-body--economy',tab==='economy');document.querySelectorAll('.journal-tabs button').forEach(b=>{const active=b.dataset.tab===tab;b.classList.toggle('active',active);b.setAttribute('aria-selected',active)});const [title,text]=objective();
    ...
    if(tab==='skills'){el.journalBody.innerHTML=window.EVERLIGHT_SKILLS.render(S);}
  ...
  function handleUiAction(target){
    const hearthAction=target.closest('[data-hearth]');if(hearthAction){const a=hearthAction.dataset.hearth;if(a==='keep'){closeInteraction();return;}if(a==='offer'){openInteraction('A letter that should not exist',HEARTH.invitation(S));return;}if(['resume'].includes(a)||a.startsWith('promise:')){if(!HEARTH.inside(S)&&enemies.some(e=>!e.dead&&e.engaged&&distance(e,S)<800)){toast('Find a safe moment before following the memory.');return;}}else{const id=a.includes(':')?a.split(':')[1]:a;const o=HEARTH.objects(S).find(o=>o.hearth===(a.startsWith('resolve:')||a.startsWith('attune:')?'alden':id));if(!['hall'].includes(a)&&(!o||distance(S,o)>120))return;}HEARTH.action(S,a,hearthHooks());updateHUD();return;}
    const tradeView=target.closest('[data-merchant-view]');if(tradeView&&merchantContext){merchantView={mode:tradeView.dataset.merchantView==='sell'?'sell':'buy',pendingUid:null,message:''};renderMerchant();return;}
    if(target.closest('[data-cancel-sale]')&&merchantContext){merchantView.pendingUid=null;renderMerchant();return;}
    const sale=target.closest('[data-sell]'),confirmSale=target.closest('[data-confirm-sell]');if((sale||confirmSale)&&merchantContext&&!el.interactionPanel.classList.contains('is-hidden')){const id=sale?.dataset.sell||confirmSale.dataset.confirmSell,item=itemByUid(id);if(confirmSale&&merchantView.pendingUid!==id)return;if(sale&&item&&window.EVERLIGHT_MERCHANTS.requiresConfirmation(item)){merchantView.pendingUid=id;}else{const result=window.EVERLIGHT_MERCHANTS.sell(S,id);merchantView.message=result.message;merchantView.pendingUid=null;if(result.ok){save();sfx('coin');}}merchantView.mode='sell';renderMerchant();updateHUD();return;}
    const storyTrack=target.closest('[data-story-track]');if(storyTrack&&STORY.unlocked(S)){const id=storyTrack.dataset.storyTrack;if(id==='main'||STORY.sides.some(q=>q.id===id)){S.story.tracked=id;trackedDoor=null;save();closeJournal();closeInteraction();updateHUD();toast(STORY.objective(S).join(' · '));}return;}
    const storyAction=target.closest('[data-story-action]');if(storyAction){const id=storyAction.dataset.storyPerson,p=STORY.people[id];if(p&&p.zone===S.zone&&distance(S,p)<120&&STORY.action(S,id,storyAction.dataset.storyAction,storyHooks())){closeInteraction();save();updateHUD();}return;}
    if(target.closest('[data-boss-trial]')){if(S.zone!=='moonfall'||!S.opened.includes('hollow-warden')||S.opened.includes('warden-rematch-v27')||enemies.some(e=>e.kind==='warden'&&!e.dead))return;closeInteraction();enemies.push(configureBoss(makeEnemy('warden',1570,560,109),true));S.x=1260;S.y=650;zoneGrace=1.5;toast('Warden’s Remembrance · Recommended level 6');return;}
    ...
    const buy=target.closest('[data-buy]');if(buy&&merchantContext){const item=ITEM_TEMPLATES[buy.dataset.buy];if(!item||!SHOP_STOCK[merchantContext.kind].includes(item.id))return;const price=priceFor(item);if(S.gold>=price){S.gold-=price;const gained=addItem(item.id);merchantView.message=`Purchased ${gained.name} for ${price} gold.`;save();renderMerchant();updateHUD();}return;}
    ...
    const equip=target.closest('[data-equip]');if(equip){const result=window.EVERLIGHT_EQUIPMENT.equip(S,equip.dataset.equip,equip.dataset.slot);gearView.message=result.message;if(result.ok){S.hp=Math.min(S.hp,combatStats().maxHp);save();}renderJournal('gear');updateHUD();return;}
    const unequip=target.closest('[data-unequip]');if(unequip){const result=window.EVERLIGHT_EQUIPMENT.unequip(S,unequip.dataset.unequip);gearView.message=result.message;if(result.ok){S.hp=Math.min(S.hp,combatStats().maxHp);save();}renderJournal('gear');updateHUD();return;}
    ...
    const use=target.closest('[data-use]');if(use){const index=S.inventory.findIndex(i=>i.uid===use.dataset.use),item=S.inventory[index];if(item?.type==='Consumable'){if(!(item.heal&&S.hp<combatStats().maxHp)&&!(item.mana&&S.mana<combatStats().maxMana)){gearView.message='Already restored. Item kept in your pack.';renderJournal('gear');return;}if(item.heal)S.hp=Math.min(combatStats().maxHp,S.hp+item.heal*(1+skillBonuses().healing));if(item.mana)S.mana=Math.min(combatStats().maxMana,S.mana+item.mana);S.inventory.splice(index,1);gearView.selectedUid=null;gearView.message=`${item.name} used.`;save();renderJournal('gear');updateHUD();}return;}
    const skill=target.closest('[data-skill]');if(skill){const result=window.EVERLIGHT_SKILLS.buy(S,skill.dataset.skill);if(result.ok){sfx('success');save();}toast(result.message);renderJournal('skills');updateHUD();return;}
    ...
    const join=target.closest('[data-join]');if(join&&!S.faction){S.faction=join.dataset.join;S.factionRep[S.faction]=25;if(S.faction==='ashen'){S.maxMana+=18;S.mana=combatStats().maxMana;}S.mainStep=Math.max(S.mainStep,5);sfx('success');save();closeInteraction();subtitle(`${FACTIONS[S.faction].name} welcomes you. The Moonfall road is open.`,3);return;}
    ...
    const econ=target.closest('[data-economy]');if(econ){if(econ.dataset.economy==='collect'){const amount=Math.max(0,S.treasury);S.gold+=amount;S.treasury-=amount;estateView.message=amount?`${amount} gold transferred to your purse.`:'No positive treasury balance to transfer.';}else{const settled=advanceDay();estateView.message=settled?`Day ${S.day} settled. Profits and expenses are recorded below.`:'Keep adventuring to complete this business day. Resting cannot generate rent.';}save();updateHUD();renderJournal('economy');return;}
  ...
  function drawWorld(){if(HEARTH.inside(S)){HEARTH.drawWorld(ctx,S,performance.now());return;}const z=zone(),interior=INTERIOR_LAYOUTS[S.zone],art=interior&&imageReady.interiors[S.zone]?interiorArt[S.zone]:z.type==='town'&&imageReady.bg?bg:z.type==='wild'&&imageReady.greenwake?greenwakeBg:z.type==='ruins'&&imageReady.moonfall?moonfallBg:null;drawWorldSurroundings(z,art);if(z.type==='room'){if(INT&&z.theme!=='dungeon')INT.draw(ctx,roomOptions());else EX.drawRoom(ctx,z,solidRects());}else if(z.type==='atlas')drawAtlasWorld(z.area,z);else{if(art){ctx.drawImage(art,0,0,z.width,z.height);ctx.fillStyle=z.type==='town'?'rgba(5,28,22,.08)':z.type==='interior'?'rgba(3,10,9,.03)':'rgba(2,18,15,.05)';ctx.fillRect(0,0,z.width,z.height);}else{const grad=ctx.createLinearGradient(0,0,0,z.height);grad.addColorStop(0,z.tint);grad.addColorStop(1,'#071713');ctx.fillStyle=grad;ctx.fillRect(0,0,z.width,z.height);drawGroundPattern(z);}}const development=landBuilding(S.zone);if(development)EX.drawHouse(ctx,development);for(const d of decor)drawDecor(d);}
  ...
  function draw(){ctx.setTransform(1,0,0,1,0,0);ctx.clearRect(0,0,VIEW_W,VIEW_H);const sx=S.settings.reducedMotion?0:random(-shake,shake),sy=S.settings.reducedMotion?0:random(-shake,shake);ctx.save();ctx.translate(-camera.x+sx,-camera.y+sy);drawWorld();const objects=objectsForZone(),isNpc=o=>o.kind==='hearth'&&o.actor||['npc','shop','rest','stable','faction','localNpc','atlasNpc','atlasShop'].includes(o.kind);for(const o of objects.filter(o=>!isNpc(o)))drawObject(o);const actors=[...objects.filter(isNpc).map(o=>({y:o.y,type:'npc',o})),...enemies.filter(e=>!e.dead).map(e=>({y:e.y,type:'enemy',o:e})),...(companionActive()?[{y:companion.y,type:'companion'}]:[]),{y:S.y,type:'hero'}].sort((a,b)=>a.y-b.y);for(const a of actors){if(a.type==='hero')drawHero();else if(a.type==='companion')drawCompanion();else if(a.type==='npc')drawObject(a.o);else drawEnemy(a.o);}drawLoot();drawProjectiles();drawHearthHazards();drawEffects();ctx.restore();if(hurtFlash>0){ctx.fillStyle=`rgba(255,55,40,${hurtFlash*.35})`;ctx.fillRect(0,0,VIEW_W,VIEW_H);}if(screenFlash>0){ctx.fillStyle=`rgba(170,255,235,${screenFlash*.42})`;ctx.fillRect(0,0,VIEW_W,VIEW_H);}}

  function update(dt){if(gameplayBlocked())return;S.playTime+=dt;const adventuring=Math.hypot(input.x,input.y)>.1||['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','KeyW','KeyA','KeyS','KeyD'].some(k=>keys.has(k))||enemies.some(e=>!e.dead&&e.engaged&&distance(e,S)<900);if(adventuring)S.economyClock=(S.economyClock||0)+dt;if(S.economyClock>=300){advanceDay();toast(`Day ${S.day} · Business accounts settled`);save();}saveClock+=dt;zoneGrace=Math.max(0,zoneGrace-dt);updatePlayer(dt);updateEnemies(dt);updateHearthCombat(dt);updateCompanion(dt);updateProjectiles(dt);updateLoot(dt);updateEffects(dt);updateInteraction();updateHUD();updateCamera(dt);updateMinimap();el.attackCooldown.style.height=`${attackCd/.42*100}%`;el.spellCooldown.style.height=`${spellCd/1.05*100}%`;el.dodgeCooldown.style.height=`${dodgeCd/.82*100}%`;presentLevelUp();if(saveClock>4){save();saveClock=0;}}
  ...
  document.addEventListener('visibilitychange',()=>{if(document.hidden){save();paused=true;return;}const modalOpen=[el.dialogue,el.journal,el.chapterComplete,el.interactionPanel,$('levelUpPanel')].some(n=>!n.classList.contains('is-hidden'));if(running&&!modalOpen){paused=false;last=performance.now();}});
  ...
  el.continueBtn.onclick=()=>{wakeAudio();beginGame(false);openReturnRecap();};
  ...
  el.keepExploringBtn.onclick=()=>{show(el.chapterComplete,false);el.gameScreen.inert=false;paused=false;STORY.init(S);S.story.tracked='main';trackedDoor=null;save();updateHUD();openJournal('quest');};
  ...
  function drawStoryObject(o){if(o.actor)drawNpc({...o,kind:'localNpc'});else STORY.drawProp(ctx,o,S);ctx.save();ctx.translate(o.x,o.y-(o.actor?86:64));ctx.fillStyle='#ffe291';ctx.shadowColor='#ffe291';ctx.shadowBlur=12;ctx.beginPath();ctx.moveTo(0,-8);ctx.lineTo(7,0);ctx.lineTo(0,8);ctx.lineTo(-7,0);ctx.closePath();ctx.fill();ctx.restore();if(distance(S,o)<190){ctx.save();ctx.font='700 12px system-ui';ctx.textAlign='center';ctx.fillStyle='#fff0b5';ctx.strokeStyle='#07130f';ctx.lineWidth=4;ctx.strokeText(o.name,o.x,o.y-102);ctx.fillText(o.name,o.x,o.y-102);ctx.restore();}}
  function startQaStory(){S=defaultSave();S.style='arcanist';S.storyChoice='mercy';S.mainStep=8;S.chapterComplete=true;S.opened=['hollow-warden'];S.level=6;S.maxHp=160;S.hp=160;S.maxMana=100;S.mana=100;S.skillPoints=6;S.gold=850;for(const id of ['briar_edge','leather_coat','oak_shield','trail_helm','scout_belt','lantern_charm','aether_lens']){const i=addItem(id);if(i.type!=='Quest')window.EVERLIGHT_EQUIPMENT.equip(S,i.uid);}beginGame(false);enterZone('moonfall',{x:1190,y:755},false);enemies=[];updateHUD();}
  let debugHarnessState = null;
  if(['localhost','127.0.0.1'].includes(location.hostname)) window.__EVERLIGHT_DEBUG__ = {
    navigation:{
      start:()=>beginGame(),
      setState:patch=>{for(const n of [el.dialogue,el.journal,el.chapterComplete,el.interactionPanel,$('levelUpPanel')])show(n,false);el.gameScreen.inert=false;S=migrateSave({...defaultSave(),...patch});enterZone(S.zone,{x:S.x,y:S.y},false);paused=false;},
      ...
      get:()=>({state:structuredClone(S),enemies:structuredClone(enemies),projectiles:structuredClone(projectiles),loot:structuredClone(loot),target:navigationTarget(),hazards:structuredClone(hearthHazards),camera:{...camera},paused,sprinting,doorCooldown}),
    ...
    spawnEnemy: (kind='briar',x=S.x+100,y=S.y) => {const e=makeEnemy(kind,x,y,enemies.length);enemies.push(kind==='warden'?configureBoss(e):e);},
  ...
  function startQaEconomy(){startQaInterior('northford');S.gold=18000;S.atlasDiscovered=[ATLAS.firstArea];S.materials.briarFiber=30;estateView.view='market';estateView.selectedId=null;updateHUD();openJournal('economy');}
  function startQaBoss(){S=defaultSave();S.style='arcanist';S.mainStep=6;S.level=6;S.maxHp=160;S.hp=160;S.maxMana=100;S.mana=100;S.gold=1000;S.skills=['aether_surge'];for(const id of ['briar_edge','leather_coat','oak_shield','trail_helm','scout_belt','lantern_charm']){const item=addItem(id);window.EVERLIGHT_EQUIPMENT.equip(S,item.uid);}S.hp=combatStats().maxHp;beginGame(false);enterZone('moonfall',{x:1120,y:650},false);enemies=enemies.filter(e=>e.kind==='warden');paused=false;show(el.toast,false);updateHUD();}
  function startQaDesign(theme){startQaInterior('northford');S.gold=100000;ECON.buy(S,'land:northford',ESTATES);ECON.develop(S,'land:northford',theme,ESTATES);if(S.properties['land:northford']?.businessType)enterZone('business@land:northford',null,false);else enterZone(EX.homes(ATLAS.get(ATLAS.firstArea))[0].id,null,false);show(el.toast,false);}
  function startQaEquipment(){
    ...
    for(const id of ['briar_edge','wayfarer_bow','leather_coat','warden_plate','lantern_charm','saltglass_idol','oak_shield','trail_helm','scout_belt','tonic','aether_lens'])addItem(id);
    S.equipment.Armor=S.inventory.find(i=>i.id==='leather_coat').uid;updateHUD();openJournal('gear');
  }

  $('levelUpContinue').onclick=()=>dismissLevelUp();$('levelUpSkills').onclick=()=>dismissLevelUp(true);
  applySettings();updateFullscreenButton();
  addEventListener('load',()=>setTimeout(()=>{show(el.loading,false);const local=['127.0.0.1','localhost'].includes(location.hostname),qa=new URLSearchParams(location.search).get('qa');if(local&&qa==='hearth'){startQaStory();S.hearth.offered=true;openReturnRecap();return;}if(local&&qa==='reading'){startQaStory();S.story.side.silverleaf={status:'active',progress:0};S.story.tracked='silverleaf';enterZone('atlas_greenwake_02',{x:610,y:790},false);return;}if(local&&qa==='story'){startQaStory();return;}if(local&&qa==='level'){startQaStory();gainXp(S.level*80);return;}if(local&&qa==='boss'){startQaBoss();return;}if(local&&qa?.startsWith('design-')){startQaDesign(qa.slice(7));return;}if(local&&qa==='economy'){startQaEconomy();return;}if(local&&qa==='gear'){startQaEquipment();return;}if(local&&qa==='combat'){startQaCombatShowcase();return;}if(local&&['atlas','atlas-property','atlas-board','atlas-route'].includes(qa)){startQaAtlasShowcase(qa);return;}if(local&&qa?.startsWith('interior-')){const id=qa.replace('interior-','');if(INTERIOR_LAYOUTS[id]){startQaInterior(id);return;}}if(local&&qa==='town'){startQaInterior('northford');return;}if(local&&qa?.startsWith('room-')){const id=qa.slice(5);if(EX.roomDefs[id]){startQaInterior(id);return;}}show(el.titleScreen,true);if((localStorage.getItem(SAVE_KEY)||localStorage.getItem(LEGACY_SAVE_KEY))&&S.style)show(el.continueBtn,true);},520));
})();
